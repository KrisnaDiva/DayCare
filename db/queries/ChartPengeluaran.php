...
$statement = $koneksi->prepare("SELECT DATE(tanggal) as transactionDay, SUM(total_pengeluaran) as totalPayment
    FROM pengeluaran
    WHERE status = 'diterima' AND tanggal >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
...
$statement = $koneksi->prepare("SELECT DATE(tanggal) as transactionDay, SUM(total_pengeluaran) as totalPayment
    FROM pengeluaran
    WHERE status = 'diterima' AND WEEKOFYEAR(tanggal) >= WEEKOFYEAR(DATE_SUB(CURDATE(), INTERVAL 4 WEEK))
    GROUP BY WEEKOFYEAR(tanggal)");
$statement->execute();
$rows = $statement->fetchAll(PDO::FETCH_ASSOC);
...
$statement = $koneksi->prepare("SELECT DATE_FORMAT(tanggal, '%Y-%m') as transactionMonth, SUM(total_pengeluaran) as totalPayment
    FROM pengeluaran
    WHERE status = 'diterima' AND tanggal >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)
...
<script>
    ...
    window.onload = function () {
        ...
        var title = "Pengeluaran Dalam Seminggu";
        renderPaymentChart(dataPoints, title);
    }
</script>