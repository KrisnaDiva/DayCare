...
?>
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
            ...
            <li class="page-item <?= $page == 1 ? 'disabled' : '' ?>">
                <a class="page-link"
                   href="testimoni.php?tingkat_kepuasan=<?= $tingkat_kepuasan ?>&page=<?= max(1, $page - 1) ?>"><</a>
            </li>
            <?php
            $start = max(1, $page - 2);
            ...
            for ($i = $start; $i <= $end; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link"
                       href="testimoni.php?tingkat_kepuasan=<?= $tingkat_kepuasan ?>&page=<?= $i ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>
            <li class="page-item <?= $page == $total_pages ? 'disabled' : '' ?>">
                <a class="page-link"
                   href="testimoni.php?tingkat_kepuasan=<?= $tingkat_kepuasan ?>&page=<?= min($total_pages, $page + 1) ?>">></a>
            </li>
            <li class="page-item <?= $page == $total_pages ? 'disabled' : '' ?>">
                <a class="page-link"
                   href="testimoni.php?tingkat_kepuasan=<?= $tingkat_kepuasan ?>&page=<?= $total_pages ?>">Last</a>
            </li>
        </ul>
    </nav>
